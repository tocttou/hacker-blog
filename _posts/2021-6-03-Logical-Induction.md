...
However, there is a sense in which logical inductors are non-dogmatic. If a theorem is not proven to be true, i.e., it is never output by our deductive process, then our logical inductor will assign it probability strictly less than 1 (and likewise for sentences proven to be false). 

To see this, suppose that our logical inductor assigns limiting probability 1 to a statement that cannot be proven to be true. Consider a trader that watches this statement and buys when the price is \$0.50, \$0.25, \$0.125, \$0.0625, …, purchasing an infinite number of copies of this sentence with only \$1. Since the statement cannot be proven to be true, there will always exist a world propositionally consistent with the deductive process in which this statement is true. In that world, the trader makes \$1 + \$1 + \$1 + …, exploiting our logical inductor. Thus our logical inductor must assign a limiting probability bounded away from 1.
